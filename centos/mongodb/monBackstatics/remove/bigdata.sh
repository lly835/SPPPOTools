...
defined_delete_date="2017-08-14"
...
#查询总的数据量
runCmd "db.action_log_new.count()" $mongodb_host $mongodb_admin_user $mongodb_admin_pass $mongodb_port $i $random  admin

if [[ $? -ne 0  ]];then
    print_log "获取action_log_new总的数据量失败,退出"
    exit
fi
totalCount=`cat /tmp/$random |grep -v bye |sed "1,3d" | grep -v help |grep -v WARNING`
rm -f /tmp/$random

print_log "###action_log_new总数据量:${totalCount}"
#查询小于${delete_date}的数量语句
runSQL='db.action_log_new.find({"sendTime":{$lt:ISODate("'${delete_date}'")}}).count()'
...
do
    ...
    print_log "获取action_log_new 小于${delete_date}的数量"
    print_log "执行语句:$runSQL"
    runCmd "${runSQL}" $mongodb_host $mongodb_admin_user $mongodb_admin_pass $mongodb_port $i $random  admin
    if [[ $? -ne 0  ]];then
        print_log "获取action_log_new 小于${delete_date}的数量失败,退出"
        exit
    fi
    maxCount=`cat /tmp/$random |grep -v bye |sed "1,3d" | grep -v help |grep -v WARNING`
    print_log "###action_log_new sendTime小于${delete_date}的数量:${maxCount}"
    rm -f /tmp/$random
    if [[ ${maxCount} -eq 0 ]]; then
        print_log "action_log_new sendTime小于${delete_date}的数据为:${maxCount},退出循环"
        break
    fi

    #执行删除
    deleteSQL='removeIdsArray=db.getCollection("action_log_new").find({"sendTime":{$lt:ISODate("'${delete_date}'")}}).limit('${deleteMaxCount}').toArray().map(function(doc) { return doc._id; });db.action_log_new.remove({_id: {$in: removeIdsArray}})'
